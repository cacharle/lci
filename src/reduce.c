...
{
    bindings->len++;
    bindings->data = realloc(bindings->data, sizeof(binding_t) * bindings->len);
    bindings->data[bindings->len - 1].name = name;
    bindings->data[bindings->len - 1].expr = expr;
}
...
bindings_search(bindings_t *bindings, char *name)
{
    for (size_t i = bindings->len; i > 0; i--)
        if (strcmp(bindings->data[i - 1].name, name) == 0)
            return bindings->data[i - 1].expr;
    return NULL;
}
...
{
    expr_t *expr = expr_clone(input_expr);
    expr_t *found;
    switch (expr->tag)
    {
    case EXPR_VAR:
